        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('classes_subjectscrud.create') }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
                        + Új Adat
                    </a>
                </div>
                            ...
                            @foreach($classes_subjects as $class_subject)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-sm">
                                        <a href="{{ route('classes_subjectscrud.edit', $class_subject->id) }}" class="text-indigo-600 hover:underline mr-4">Módosítás</a>
                                        
                                        <form action="{{ route('classes_subjectscrud.destroy', $class_subject->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Biztos vagy benne?');">
                                            @csrf
                                            @method('DELETE')
