        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('studentscrud.update', $student->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="gender" id="gender" required
                                ...
                                <option value="F" {{ old('gender', $student->gender) == 'F' ? 'selected' : '' }}>Férfi</option>
                                <option value="N" {{ old('gender', $student->gender) == 'N' ? 'selected' : '' }}>Nő</option>
                            </select>
                            @error('gender')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
