                ...
                <div class="p-6 text-gray-900">
                    <form method="GET" action="{{ request()->url() }}">
                        ...
                        <option value="{{ route('crud.index') }}">-- Adattáblák --</option>
                            ...
                            <option value="{{ route('crud.marks') }}" selected>
                                    Osztályzatok
                            </option>
                        </select>
                    </form>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-sm font-semibold text-gray-700">Tanuló_ID</th>
                                <th class="px-4 py-2 text-left text-sm font-semibold text-gray-700">Tantárgy_ID</th>
                                <th class="px-4 py-2 text-left text-sm font-semibold text-gray-700">Osztályzat</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($marks as $mark)
                                <tr>
                                    <td class="px-4 py-2 text-sm text-gray-900">{{ $mark->id }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-900">{{ $mark->student->name ?? 'N/A' }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-900">{{ $mark->subject->name ?? 'N/A' }}</td>
